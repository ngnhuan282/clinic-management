export const INTERNAL_ROLES = ["Admin", "Doctor", "Receptionist"];
...
export const homeForRole = role => INTERNAL_ROLES.includes(role) ? "/internal/dashboard" : "/booking";

export function canAccessPath(role, path) {
    if (path === "/booking") return role === "Patient";
    if (!path.startsWith("/internal")) return true;
    const page = INTERNAL_PAGES.find(item => path === item.path || path.startsWith(`${item.path}/`));
